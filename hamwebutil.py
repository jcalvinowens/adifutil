...
class WobsiteGroup(QSOGroup):
	...
	def save_all_charts(self, d):
		...
		self.save_sigstrengthrcvd_plots(filename=f"{d}/sigstrengthrcvd.svg")
		self.save_sigstrengthrel_plots(filename=f"{d}/sigstrengthrel.svg")
		self.save_clock_dist_plot(filename=f"{d}/clockdistplot.svg")
...
def parse_arguments():
	...
	parser.add_argument("-M", dest="mode", type=str, default="FT8")
	parser.add_argument("-C", dest="call", type=str, default=None)
	parser.add_argument("-G", dest="grid", type=str, default=None)
...
if __name__ == "__main__":
	...
	qsos = WobsiteGroup(QSO.load_qsos_from_files(*args.files))
	fqsos = WobsiteGroup(qsos.qsos, call=args.call, mode=args.mode, grid=args.grid)
	fqsos.save_all_charts(".")
	fqsos.save_stats(".")
